...
10. **Cuidi**: OSv provides the isolation for a single application. Because an application always uses multiple processes, OSv might provide a way to make those processes work together with fewer duplications. It may give multiple processes a way to share memory in an application. 

11. **Cuidi**: OSv could boot as fast as 5ms and could run on many hypervisors, I guess it may reduce much unnecessary libraries of Linux. Because it is unikernel, that means the OSv should be compiled together. Therefore, the developers could customize the kernel and library before using it.
